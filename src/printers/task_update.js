...
module.exports = task => {
    console.log(`\n${colors.bold.green(`${task.score} [${task.recurring_type}] (${task.recurring_count || 0}/${task.stop_recurrence_after || 0}) ${task.name}`)}\n`);
    ...
    if (task.increment_actions && task.increment_actions.length > 0) {
        let incrementActions = task.increment_actions.slice(0, 3).map(incrementAction => {
            return `${colors.bold.underline.white(new Date(incrementAction.date).toLocaleDateString('en-us'))} - ${colors.bold.yellow(incrementAction.action)}`;
        }).join('\n');
